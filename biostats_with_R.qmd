...
```{r}
t.test(drug_trial$SBP1, drug_trial$SBP2, paired = TRUE)
```
...
```{r}
hyp_chol <- glm(Hypertension ~ Cholestrol, family=binomial(logit), data=StOB)
summary(hyp_chol)
```
...
```{r}
ui_age <- glm(ui ~ age, family=binomial(logit), data=lbw_data)
summary(ui_age)
```

```{r}
exp(cbind(OR=coef(hyp_chol), confint(hyp_chol)))
```

```{r}
exp(cbind(OR=coef(ui_age), confint(ui_age)))
```

```{r}
bdp_age <- glm(ui ~ age, family=binomial(logit), data=lbw_data)
summary(ui_age)
```
